...
void loop() {
  ...
  if (!parse8Floats(line, raw)) {
    ...
    Serial.println("PRED:PARSE_ERROR,CONF:0.0,TIME_US:0,HEAP_TOTAL:0,FREE_HEAP:0,USED_HEAP:0");
    Serial.println("----------------------------------");
    return;
  }
  ...
  if (pred < 0 || pred >= N_CLASSES) {
    ...
    Serial.println("PRED:INFERENCE_ERROR,CONF:0.0,TIME_US:0,HEAP_TOTAL:0,FREE_HEAP:0,USED_HEAP:0");
    Serial.println("----------------------------------");
    return;
  }
