...
// =========================================================
// CONFIGURATION MODELE ANN STABLE BINAIRE - CICIDS2018
// =========================================================

const int N_FEATURES = 8;

// Modèle ANN binaire : sortie sigmoid unique
// output[0] = probabilité ATTACK
const float THRESHOLD = 0.5f;

// TensorArena pour TensorFlow Lite Micro
...
float means[N_FEATURES] = {
  11426.32299983408f,     // Init Bwd Win Bytes
  483.9186195453791f,     // Fwd Packet Length Max
  238.50767860423724f,    // Fwd Packet Length Std
  45204.55583750888f,     // Fwd IAT Min
  115634.06475713904f,    // Flow IAT Mean
  325708.22096282896f,    // Flow Bytes/s
  160.96377434745483f,    // Avg Packet Size
  680.4812078977933f      // Total Fwd Packets
};

float scales[N_FEATURES] = {
  17075.737219914463f,    // Init Bwd Win Bytes
  301.54054726248233f,    // Fwd Packet Length Max
  121.47205843352386f,    // Fwd Packet Length Std
  1015317.8927028695f,    // Fwd IAT Min
  1231045.1973182205f,    // Flow IAT Mean
  602644.515605166f,      // Flow Bytes/s
  46.90564382500144f,     // Avg Packet Size
  9122.483740050544f      // Total Fwd Packets
};
...
void setup() {
  ...
  Serial.println();
  Serial.println("===== ESP32 ANN STABLE BINAIRE CICIDS2018 START =====");
  Serial.println("Modele : ann_final_stable_seed84.tflite");
  Serial.println("Header : ann_model_data.h");
  ...
  if (!modelInit(ann_model_tflite, tensorArena, kTensorArenaSize)) {
    ...
    Serial.println("Essaye d'augmenter kTensorArenaSize a 96*1024 ou 128*1024");
...
void loop() {
  ...
  // Modèle binaire sigmoid :
  // sortie unique = probabilité de la classe ATTACK
  ...
  // Ligne finale parseable par ton script Python Kali
  Serial.print("PRED:");
